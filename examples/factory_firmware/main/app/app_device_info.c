...
{
    int reset_factory_switch2 = reset_factory_switch;

    switch (caller)
    {
        case AT_CMD_CALLER:
            ESP_LOGI(TAG, "AT_CMD_CALLER  get_reset_factory");
            break;
        case UI_CALLER:
            ESP_LOGI(TAG, "UI  get_reset_factory");
            esp_event_post_to(app_event_loop_handle, VIEW_EVENT_BASE, VIEW_EVENT_FACTORY_RESET_CODE, 
                              &reset_factory_switch2, sizeof(int), portMAX_DELAY);
            break;
    }

    return reset_factory_switch2;
}

esp_err_t set_reset_factory(int caller, int value)
{
    xSemaphoreTake(MUTEX_reset_factory, portMAX_DELAY);
    reset_factory_switch_past = reset_factory_switch;
...
static esp_err_t __set_reset_factory()
{
    xSemaphoreTake(MUTEX_reset_factory, portMAX_DELAY);
    if (reset_factory_switch_past != reset_factory_switch)
    {
        ...
        ESP_RETURN_ON_ERROR(storage_write(RESET_FACTORY_SK, &reset_factory_switch, sizeof(reset_factory_switch)), 
                            TAG, "set_sound cfg write err");
        reset_factory_switch_past = reset_factory_switch;
        ESP_LOGD(TAG, "set_reset_factory done: %d", reset_factory_switch);
    }
    xSemaphoreGive(MUTEX_reset_factory);
    return ESP_OK;
}
