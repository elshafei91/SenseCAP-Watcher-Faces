...
int16_t sensor_scd40_init(void)
{
    ...
    sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2); // wake up, sensor does not acknowledge the wake-up call, error is ignored
    sensirion_i2c_hal_sleep_usec(20000);

    sensirion_i2c_add_command_to_buffer(&buffer[0], 0, 0x3F86);
    ret = sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2); // stop measurement
    ESP_RETURN_ON_ERROR(ret, TAG, "stop measurement error %d!", ret);
    sensirion_i2c_hal_sleep_usec(500000);

    sensirion_i2c_add_command_to_buffer(&buffer[0], 0, 0x3646);
    ret = sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2); // init
    ESP_RETURN_ON_ERROR(ret, TAG, "init error %d!", ret);
    sensirion_i2c_hal_sleep_usec(20000);

    sensirion_i2c_add_command_to_buffer(&buffer[0], 0, 0x21B1);
    sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2); // start measurement
...
int16_t sensor_scd40_get_data_ready_flag(bool* data_ready_flag)
{
    ...
    ret = sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2);
    ESP_RETURN_ON_ERROR(ret, TAG, "write ready flag command error %d!", ret);
    sensirion_i2c_hal_sleep_usec(1000);

    ret = sensirion_i2c_read_data_inplace(SENSOR_SCD40_I2C_ADDR, &buffer[0], 2);
...
int16_t sensor_scd40_read_measurement(uint32_t *co2)
{
    ...
    ret = sensirion_i2c_write_data(SENSOR_SCD40_I2C_ADDR, &buffer[0], offset);
    ESP_RETURN_ON_ERROR(ret, TAG, "write measurement command error %d!", ret);
    sensirion_i2c_hal_sleep_usec(1000);

    ret = sensirion_i2c_read_data_inplace(SENSOR_SCD40_I2C_ADDR, &buffer[0], 6);
