...
int16_t sensor_sht41_init(void)
{
    ...
    uint8_t buffer[2] = {0};
    buffer[0] = 0x94;
    ret = sensirion_i2c_write_data(SENSOR_SHT41_I2C_ADDR, buffer, 1); // soft reset
...
int16_t sensor_sht41_read_measurement(int32_t *temperature, int32_t *humidity)
{
    ...
    // ret = sensirion_i2c_write_data(SENSOR_SHT41_I2C_ADDR, buffer, 1);
    // sensirion_i2c_hal_sleep_usec(10 * 1000);
    ...
    ret = sensirion_i2c_read_data_inplace(SENSOR_SHT41_I2C_ADDR, buffer, 4);
